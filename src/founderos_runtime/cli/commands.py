...
AtlasDiscoveryRunner = Callable[[], tuple[DiscoveryResult, TopologyGraph]]
...
def atlas_discovery_command(
    ...
    if not isinstance(result, DiscoveryResult) or not isinstance(graph, TopologyGraph):
        raise CliError("Atlas Discovery demo returned an invalid result")
    return 0, render_atlas_discovery(result, graph)
