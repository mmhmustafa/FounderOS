...
(function () {
  ...
  // Advisor AI explanation (PR-166, INSIGHT). Loads ASYNCHRONOUSLY and
  // only on request: Atlas's own answer is already on the page and never
  // waits for a model. Every failure is quiet — the panel says the
  // explanation is unavailable and the deterministic answer above stands
  // on its own. Provider errors are never shown to an operator.
  var aiPanel = document.getElementById("advisor-ai-panel");
  if (aiPanel) {
    var aiRun = document.getElementById("advisor-ai-run");
    var aiBody = document.getElementById("advisor-ai-body");
    var aiText = document.getElementById("advisor-ai-text");
    var aiStatus = document.getElementById("advisor-ai-status");
    var aiProvenance = document.getElementById("advisor-ai-provenance");
    var aiAudience = document.getElementById("advisor-ai-audience");
    var aiLanguage = document.getElementById("advisor-ai-language");
    var aiCopy = document.getElementById("advisor-ai-copy");
    var aiDownload = document.getElementById("advisor-ai-download");
    ...
    var aiExplain = function () {
      ...
      aiBusy = true;
      aiRun.disabled = true;
      var previousLabel = aiRun.textContent;
      aiRun.textContent = "Explaining…";
      aiStatus.textContent =
        "Asking the configured model to restate the answer above…";
      var csrfMatch = document.cookie.match(/(?:^|;\s*)atlas_csrf=([^;]+)/);
      fetch("/api/advisor/explain", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          conversation: aiPanel.getAttribute("data-conversation") || "0",
          audience: aiAudience ? aiAudience.value : "",
          language: aiLanguage ? aiLanguage.value : "en",
          _csrf: csrfMatch ? decodeURIComponent(csrfMatch[1]) : "",
        }),
      })
        .then(function (response) { return response.json(); })
        .then(function (data) {
          if (data && data.ok && data.text) {
            ...
            if (data.prompt_version) { bits.push("prompt " + data.prompt_version); }
            if (data.audience_label) { bits.push("for " + data.audience_label); }
            if (data.translated) { bits.push("translated into " + data.language_label); }
            if (data.redaction_summary) { bits.push("redacted before sending: " + data.redaction_summary); }
            if (data.generated_at) { bits.push("generated " + data.generated_at); }
            aiProvenance.textContent =
              "Generated by ORACLE from Atlas evidence — " + bits.join(" · ");
            aiBody.hidden = false;
            aiStatus.textContent = "";
            aiToggleActions(true);
            aiText.focus();
          } else {
            // Part 8: an operator sees no provider error, only that the
            // enhancement is unavailable and Atlas's answer is complete.
            aiBody.hidden = true;
            aiToggleActions(false);
            aiStatus.textContent =
              "An AI explanation is not available right now. Atlas's answer "
        ...
        .then(function () {
          ...
          aiRun.textContent = previousLabel;
        });
    };

    aiRun.addEventListener("click", aiExplain);
    // Switching audience or language re-explains only if an explanation
    // is already showing; otherwise the operator asks when ready.
    [aiAudience, aiLanguage].forEach(function (control) {
      if (!control) { return; }
      control.addEventListener("change", function () {
        if (!aiBody.hidden) { aiExplain(); }
      });
    });
    if (aiCopy) {
      aiCopy.addEventListener("click", function () {
        var text = aiText.textContent || "";
        if (!navigator.clipboard) { return; }
        navigator.clipboard.writeText(text)
          .then(function () { aiCopy.textContent = "Copied"; })
          .catch(function () { aiCopy.textContent = "Copy failed"; });
        window.setTimeout(function () { aiCopy.textContent = "Copy"; }, 2000);
      });
    }
    if (aiDownload) {
      aiDownload.addEventListener("click", function () {
        var lines = [
          "Atlas Advisor — AI explanation",
          aiProvenance.textContent || "",
          "",
          aiText.textContent || "",
          "",
          "Atlas's own answer, its evidence, confidence and limitations are",
          "unchanged by this explanation and remain the source of truth.",
        ];
        var blob = new Blob([lines.join("\n")], { type: "text/plain" });
        var url = URL.createObjectURL(blob);
        var link = document.createElement("a");
        link.href = url;
        link.download = "atlas-explanation.txt";
    ...
    if (aiHide) {
      aiHide.addEventListener("click", function () {
        aiBody.hidden = true;
        aiToggleActions(false);
        aiStatus.textContent = "";
        aiRun.focus();
